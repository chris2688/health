...
# 공통 헤더 HTML (index-v2.html과 동일)
COMMON_HEADER_HTML = '''<!DOCTYPE html>
<html lang="ko">
<head>
    <meta charset="UTF-8">
...
def rebuild_category_file(filepath):
    ...
    try:
        ...
        # 메인 아이콘 추출
        icon_match = re.search(r'<div class="main-icon">(.*?)</div>', old_content)
        icon = icon_match.group(1) if icon_match else "❤️"
        
        # 제목 텍스트 추출
        h2_match = re.search(r'<h2>(.*?)</h2>', old_content)
        page_title = h2_match.group(1) if h2_match else "질환 정보"
        
        # 카드 그리드 전체 추출 (더 넓은 범위)
        cards_match = re.search(r'<div class="health-cards-grid">(.*?)</div>\s*</div>\s*</div>', old_content, re.DOTALL)
        if not cards_match:
            cards_match = re.search(r'<div class="health-cards-grid">(.*?)</div>\s*</div>', old_content, re.DOTALL)
        
        cards_html = cards_match.group(1).strip() if cards_match else ""
        ...
        # HTML 생성
        header = COMMON_HEADER_HTML.format(
            title=title,
            color1=color1,
            color2=color2
        )
...
{cards_html}
    ...
    <script>
        ...
        new_html = header + content
        
        # 파일 저장
        with open(filepath, 'w', encoding='utf-8') as f:
            f.write(new_html)
        
        print(f"  ✅ 재생성 완료! (아이콘: {icon}, 카드 수: {cards_html.count('<a href=')}개)")
        return True
...
def rebuild_sub_file(filepath):
    ...
    try:
        ...
        # 페이지 제목 추출
        page_title_match = re.search(r'<h1 class="page-title">(.*?)</h1>', old_content)
        page_title = page_title_match.group(1) if page_title_match else "건강 정보"
        
        # 뒤로가기 링크 추출
        back_link_match = re.search(r'href="(category-[^"]+\.html)"', old_content)
        back_link = back_link_match.group(1) if back_link_match else "index-v2.html"
        
        # 글 목록 추출
        articles_match = re.search(r'<div class="content">(.*?)</div>\s*</div>', old_content, re.DOTALL)
        articles_html = articles_match.group(1).strip() if articles_match else ""
...
'''
        ...
        # 헤더 닫는 태그 전에 추가 스타일 삽입
        header = header.replace('</head>', sub_styles + '</head>')
...
{articles_html}
    ...
    <script>
        ...
        article_count = articles_html.count('<article')
        print(f"  ✅ 재생성 완료! (글 {article_count}개)")
        return True
