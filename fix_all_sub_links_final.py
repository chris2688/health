...
def fix_all_sub_links(filepath):
    """모든 href="#" 링크를 sub-*.html로 수정"""
    ...
    try:
        ...
        # 패턴: <a href="#" ... class="health-card" ... > ... <h3>카테고리명</h3> ... </a>
        # health-card 클래스를 가진 링크 중 href="#"인 것 찾기
        pattern = r'(<a href="#"([^>]*class="health-card"[^>]*>.*?<h3>([^<]+)</h3>.*?</a>))'
        
        def replace_link(match):
            full_link = match.group(1)
            category_text = match.group(3).strip()
        ...
        # data-category와 onclick이 있는 링크도 처리
        pattern2 = r'<a href="#" data-category="([^"]*)" onclick="[^"]*"([^>]*>.*?<h3>([^<]+)</h3>.*?</a>)'
        
        def replace_link2(match):
            category = match.group(1) or match.group(3).strip()
            category_text = category if category else match.group(3).strip()
            rest = match.group(2)
            
            sub_file = find_sub_file(category_text)
            return f'<a href="{sub_file}"{rest}'
...
def main():
    """메인 실행"""
    print("=" * 60)
    print("🔧 모든 카테고리 파일의 서브 링크 수정")
    print("=" * 60)
    print("\n💡 수정 사항:")
    print("   href='#' 링크를 sub-*.html로 변경\n")
    
    print("📝 파일 수정 중...\n")
    fixed_files = []
    
    for file in CATEGORY_FILES:
        if fix_all_sub_links(file):
