...
def main():
    ...
    for sub in SUBCATEGORIES:
        subcategory_cards += f'''                <a href="{sub['file']}" class="health-card" style="--card-color-1:{sub['color1']}; --card-color-2:{sub['color2']};">
...
'''
    ...
    # HTML 생성
    html = HTML_TEMPLATE.replace('{SUBCATEGORY_CARDS}', subcategory_cards.strip())
    
    # 파일 저장
    with open('category-심혈관질환.html', 'w', encoding='utf-8') as f:
        f.write(html)
    
    print("✅ category-심혈관질환.html 재생성 완료!")
    print(f"   - 서브 카테고리: {len(SUBCATEGORIES)}개")
