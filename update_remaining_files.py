...
# 이미 완료된 파일
completed_files = ['sub-역류성식도염.html', 'sub-고혈압.html', 'sub-당뇨.html']

# 개선된 loadPosts 함수 코드 (sub-고혈압.html에서 복사)
IMPROVED_LOADPOSTS = '''        // 워드프레스 REST API로 포스트 목록 가져오기 (여러 카테고리 지원)
...
def update_file(filepath):
    ...
    try:
        ...
        # 시작 부분 찾기
        start_marker = "// 워드프레스 REST API로 포스트 목록 가져오기"
        if start_marker not in content:
            print(f"  ⚠️  loadPosts 함수를 찾을 수 없음")
            return False
        
        # loadPosts 함수의 시작과 끝 찾기
        start_idx = content.find(start_marker)
        if start_idx == -1:
            print(f"  ⚠️  시작 마커를 찾을 수 없음")
            return False
        
        # 함수의 끝 찾기 (다음 함수나 스크립트 끝)
        # "        }" 패턴으로 함수 끝 찾기
        ...
        for i, line in enumerate(lines):
            if 'async function loadPosts' in line:
                in_function = True
            if in_function:
        ...
        if end_idx == start_idx:
            print(f"  ⚠️  함수 끝을 찾을 수 없음")
            return False
        
        # 함수 교체
        old_function = content[start_idx:end_idx]
        new_content = content[:start_idx] + IMPROVED_LOADPOSTS + content[end_idx:]
        
        # loadPosts 호출 부분 수정
        new_content = new_content.replace('loadPosts(categorySlug);', 'loadPosts([categorySlug], pageTitle);')
        new_content = new_content.replace('loadPosts(categorySlug)', 'loadPosts([categorySlug], pageTitle)')
